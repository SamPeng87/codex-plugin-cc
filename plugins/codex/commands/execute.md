...
`codex:code-executor` is a subagent, not a skill — do not call `Skill(codex:code-executor)` (no such skill) or `Skill(codex:execute)` (that re-enters this command and hangs the session).

The subagent calls `node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" execute <effective arguments>` and returns stdout as-is.
...
- If neither flag is present, append `--background`. Coding runs are potentially long and must not depend on Claude's foreground Bash lifetime.
