...
The subagent calls `node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" execute-fix <effective arguments>` and returns stdout as-is.
...
- If neither flag is present, append `--background`. Test-fix runs are potentially long and must not depend on Claude's foreground Bash lifetime.
